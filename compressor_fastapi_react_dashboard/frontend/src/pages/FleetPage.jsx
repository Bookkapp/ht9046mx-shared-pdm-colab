...
export default function FleetPage() {
  ...
  return <Shell active="fleet" error={error} updatedAt={updatedAt} onRefresh={refresh} refreshing={refreshing}>
    ...
    <section className="stats-grid">
      <Stat label="MySQL machines" value={summary.mysql_machines ?? '—'} detail={`${summary.data_sources_available || 0} machines with telemetry`} />
    ...
    <section className="directory-panel">
      ...
      <div className="machine-grid">{machines.map((machine) => {
        ...
        return <a className="machine-card" href={`#/machines/${machine.machine_id}`} key={machine.machine_id}>
          <div className="machine-card-head"><div><span className={`source-light ${machine.data_source_available ? 'online' : ''}`} /><strong>{machine.machine_id}</strong><small>MySQL telemetry</small></div><Badge tone={tone(machine.lifecycle_state)}>{lifecycleLabel(machine.lifecycle_state)}</Badge></div>
          ...
          <div className="machine-card-foot"><span>Latest database event</span><small>{dateTime(latest?.event_time || machine.latest_source_at)}</small></div>
        </a>
      })}</div>
